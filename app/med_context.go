...
func NewMedContext(ctx context.Context) MedContext {
	...
	if trace.TraceId == "" {
		...
		// 不影响原有的方法使用TraceId。
		trace.TraceId = utils.TraceId()
		ctx = context.WithValue(ctx, xtrace.TraceIdName, trace.TraceId)
	}
	return MedContext{
		ctx:     ctx,
...
// 创建带链路跟踪信息的HTTP Header
func CreateHTTPHeader(ctx *MedContext) map[string]string {
	return xtrace.CreateHTTPHeader(ctx.ctx)
}

// 创建带 traceId 的 grpc context
func CreateGRPCContext(ctx *MedContext) context.Context {
	gRpcCtx, _ := context.WithTimeout(ctx.ctx, 15*time.Second)
	if nil != gRpcCtx {
		gRpcCtx = metadata.NewOutgoingContext(gRpcCtx, metadata.Pairs(utils.GrpcCtxTraceIdKey, ctx.GetTraceId()))
	}
	return gRpcCtx
}
